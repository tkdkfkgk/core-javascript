...
// first()를 실행하면 return second로 second함수 자체가 실행이 되는데 이 값을 실행시키려면 first()()하면되지만 이거보다 좋은건 변수에 넣는거다
// 클로저 사용 이유 > 전역의 오염을 막기 위해 > 변수 보호
function first(){
  let x=10;
...
// function execution context > 바깥 환경을 지켜보고있다가 본인에게 없는 것을 부모에게서 찾는다
// 자바스크립트는 함수=값 으로 인식
